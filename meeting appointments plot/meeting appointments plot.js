...
function MeetingVis() {
    ...
    this.render = function() {
		...
		var tool_tip=d3.tip().attr('class','d3-tip')
					 .offset([-8,0])
					 .html(function(d){
						 var _html="<table>"+"<tr>"+"<th>Meeting ID:</th><td>"+d.id+"</td>"+"</tr><tr>"+
						 "<th>Appointment:</th><td>"+d.appointment+"</td>"+"</tr><tr>"+
						 "<th>Start:</th><td>"+d.start+"</td>"+"</tr><tr>"+
						 "<th>End:</th><td>"+d.end+"</td>"+"</tr>"+"</table>";
						 return _html;
						}
						);
        ...
        wait_marks
            .transition().delay(wait_marks.exit().empty() ? 0 : 500).duration(500)
        ...
        wait_marks.enter().append("rect")
			...
			.transition()
                .delay(!wait_marks.exit().empty()*500 + !wait_marks.empty()*500)
                .duration(500)
        ...
        meet_marks.enter().append("rect")
			...
			.on('mouseout',tool_tip.hide)
			.style('fill-opacity',0)
            .transition().delay(!meet_marks.exit().empty()*500 + !meet_marks.empty()*500).duration(500)
                .style('fill-opacity',1);
    ...
    this.clickMark = function(d) {
        var html = "MEETING ID: " + d.id;
        html += "APPOINTMENT TIME: " + d.appointment;
        html += "START TIME: " + d.start;
        html += "END TIME: " + d.end;
